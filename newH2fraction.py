...
path_out                      = '/scratch2/dicerbo/destr/three/'
# global variables
redshift          = 19.0
Hubble            = 0.72
h2                = Hubble * Hubble
    ...
    for p in lst:
        ...
        if press*mu_c*PROTONMASS/T_a < 10.**Dens.min() or press*mu_c*PROTONMASS/T_a > 10.**Dens.max() and T_a > 1.e3:
            ...
            if check_f > 5.e-9:
                ...
                if frac < 0.:
                    line = '#error ad press %g and temperature %g\n'%(np.log10(press), np.log10(T_a)); fp.write(line)
            elif index == 10:
...
def time_int(press, Pmass, Density, T_a, tcool, fh2, time, tstep, path):
    ...
    while t < time:
        if ma <= 0.:
            ...
            if press*mu_c*PROTONMASS/Temp > dmin and press*mu_c*PROTONMASS/Temp < dmax:
            ...
            elif press*mu_c*PROTONMASS/Temp > dmax and t > tstep - 1.:
                frac_h2 = ftemp
                tcooling = ttemp
            elif press*mu_c*PROTONMASS/Temp > dmax and t == 0:
                frac_h2 = FitTDelta(Temp, dmax, fh2) #WARNING: pensare bene a che densita mettere dentro in FitTDelta
                tcooling = FitTDelta(Temp, dmax, tcool)
            else:
                print 'error'
                return -1.
