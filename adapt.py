...
path_out                      = '/scratch2/dicerbo/destr/adapt/'
# global variables
redshift          = 19.0
Hubble            = 0.72
h2                = Hubble * Hubble
...
def MolecularProfileTc():
    ...
    first_tstep = 4.e3   #timestep (smaller then minimum of tcool matrix)
    t_adapt(first_tstep)
    tstep = adapt
    ...
    for p in lst:
        ...
        if press*mu_c*PROTONMASS/T_a < 10.**Dens.min() or press*mu_c*PROTONMASS/T_a > 10.**Dens.max():
            ...
            if check_f > 5.e-9:
                ...
                wr = open(fname, 'w')
                wr.write(header)
                line = '%e\t%e\t%e\n'%(0,np.log10(Rho_a), np.log10(T_a))
                line += '%e\t%e\t%e\n'%(8e3,np.log10(Rho_a), np.log10(T_a))
                line += '%e\t%e\t%e\n'%(16e3,np.log10(Rho_a), np.log10(T_a))
                wr.write(line)
